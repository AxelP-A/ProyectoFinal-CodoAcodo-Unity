...
public class BringerOfDeath : BaseEnemy
{
	...
	void DetectingEnemy()
    {
		Debug.Log("Al menos entro al método, y fightStarted es " + fightStarted);
        RaycastHit[] hit;

        hit = Physics.RaycastAll(transform.position, Vector3.forward, 3.0f);
        ...
        foreach (RaycastHit rayhit in hit)
        {
            ...
            {
                fightStarted = true;
				Debug.Log("ENCONTRÉ ALK JUGADOR");
            }
			if (rayhit.transform.gameObject.tag == "Player")
            {
                fightStarted = true;
				Debug.Log("ENCONTRÉ AL JUGADOR wii");
            }
